...
from binaries import logger, Google_Sheets, gs_APIError, gs_NoWS, GOOGLE_API_RATE_LIMIT_EMAIL, default_all_df, default_country_df, default_error_df, developer_bot_email
...
def Create_Default_Sheet(spreadsheet,name):
    ...
    while True:
        try:
            worksheet = spreadsheet.add_worksheet(title=name, rows="5000", cols="13")
            break
        except gs_APIError as e:

...
class Gbr0002Spider(GGVenturesSpider):
    name = 'gbr_0002'
    country = 'United Kingdom'
    start_urls = ["https://www.hult.edu/en/contact/"]
    # eventbrite_id = 47699605203
    # TRANSLATE = True

    # handle_httpstatus_list = [301,302,403,404,429]

    static_name = "Ashridge Business School"
    ...
    def parse_code(self,response):
        ...
        ####################
            ...
            for link in self.events_list(event_links_xpath="//div[contains(@class,'StyledCardContent')]//a"):
                self.logger.debug(f"LINK: |{link}|")
                self.getter.get(link)
                if self.unique_event_checker(url_substring=['hult.edu']):
                    ...
                    item_data['event_desc'] = self.scrape_xpath(xpath_list=["//div[contains(@class,'StyledSectionDescription')]"],method='attr')
                    item_data['event_date'] = self.scrape_xpath(xpath_list=["//div[contains(@class,'StyledCardDescription')]"],method='attr')
                    item_data['event_time'] = self.scrape_xpath(xpath_list=["//div[contains(@class,'StyledCardDescription')]"],method='attr')
